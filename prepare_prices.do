...
foreach x of local flistWK {
		local flistwk "`flistwm' "$dir_wk/`x'""
}

foreach x of local flistHK {
		local flisthk "`flisthk' "$dir_hk/`x'""
}
...
sort ergg_1km
merge m:1 ergg_1km using "F:\Allgemein\Data\Immoscout\grid.coordinaten.dta"
tab _merge
keep if _merge==3
drop _merge
sort erg_amd
merge m:1 erg_amd using "F:\Seidel\research\Gabriel Ahlfeldt\Regional migration\data\Centroids_CBDv1_mean.dta"
tab _merge
drop _merge
rename x d_long
...
gen seCBD =.
forval num = 1/141{
*reghdfe lrent ldist Mnebenkosten Dnebenkosten Mbaujahr Dbaujahr Mwohnflaeche Dwohnflaeche Metage Detage Manzahletagen Danzahletagen Mzimmeranzahl Dzimmeranzahl Mschlafzimmer Dschlafzimmer Mbadezimmer Dbadezimmer Mimmobilientyp Dimmobilientyp Mbalkon Dbalkon Mgarten Dgarten Mkeller Dkeller Mausstattung Dausstattung Mheizungsart Dheizungsart if region == `num', abs(FE=year)
...
*reghdfe lpriceqm ldist Mwohnflaeche Dwohnflaeche Mzimmeranzahl Dzimmeranzahl Mimmobilientyp Dimmobilientyp Mkeller Dkeller Mheizungsart Dheizungsart if region == `num', abs(FE=year)
...
// PLOT CBD GRADIENTS
preserve
*reg lrent Mnebenkosten Dnebenkosten Mbaujahr Dbaujahr Mwohnflaeche Dwohnflaeche Metage Detage Manzahletagen Danzahletagen Mzimmeranzahl Dzimmeranzahl Mschlafzimmer Dschlafzimmer Mbadezimmer Dbadezimmer Mimmobilientyp Dimmobilientyp Mbalkon Dbalkon Mgarten Dgarten Mkeller Dkeller Mausstattung Dausstattung Mheizungsart Dheizungsart if region == `num', abs(year)
*reg lpriceqm ldist Mwohnflaeche Dwohnflaeche Mzimmeranzahl Dzimmeranzahl Mimmobilientyp Dimmobilientyp Mbalkon Dbalkon Mgarten Dgarten Mkeller Dkeller Mheizungsart Dheizungsart if region == `num', abs(year)
reg lpriceqm ldist Mwohnflaeche Dwohnflaeche Mzimmeranzahl Dzimmeranzahl Mimmobilientyp Dimmobilientyp Mkeller Dkeller Mheizungsart Dheizungsart if region == `num', abs(year)
predict resid, res
gen lDB = round(ldist, 0.1)
keep if region ==`num'
collapse (mean) resid (count) W=lpriceqm, by(lDB)
twoway (scatter resid lDB, mcolor(black)) (lfit resid lDB [w=W] , lcolor(red) ), xline(0, lcolor(red) lpattern(dash))  title("RAM = `num'")
graph export "gCBD_`num'_buy.png", width(2400) height(1200) replace
restore
// ADDED ELEMENTS END
