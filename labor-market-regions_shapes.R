...
# district shape
districts = st_read("../flattening-the-curve/data/geodata/admin-areas/districts.gpkg")
districts$state = NULL

# labor market regions (LMRs)
lmrs = read.csv2(
  "Labor-Market-Regions_Kostfeld-Werner-2012.csv",
...
# the shapes of the constitute districts:
# by aggregating the polygons of the the member districts

# territorial changes prevent us from perfect merges
## there are districts that are merged into another district or dissolved, or went
...
# compiled from bbsr.bund.de's "Umstiegsschlüssel für konsistente Zeitreihe" (see below),
# Google/Wikipedia
...
stopifnot(
  "There are still old districts that need to be taken care" = nrow(districts[!districts$did %in% new_lmrs$district_id, ]) == 0L
)

# labor market regions updated for the 2019 district border definitions
write.csv2(new_lmrs, "Labor-Market-Regions_Kostfeld-Werner-2012_2019.csv",
  quote = FALSE, row.names = FALSE
)

## construct the shapes of the labor market regions from the constituent districts'
...
# write the shapes
st_write(lmrs_shape, 'labor-market-regions_Kosfeld-Werner-2012_2019.gpkg')
...
fpath = sub("(.+)(\\?.+)", "\\1", basename(url))
if (!file.exists(fpath)) download.file(url, fpath)
ref_year = 2019
sheets = readxl::excel_sheets(fpath) |> trimws()
# there is a typo in the sheet names
# please manually rename sheet "2016-2018" to "2016-2019" in the excel file, if the publishers do not correct it
