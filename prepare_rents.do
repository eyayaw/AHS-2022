...
foreach x of local flistWM {
		local flistwm "`flistwm' "$dir_wm/`x'""
}

foreach x of local flistHM {
		local flisthm "`flisthm' "$dir_hm/`x'""
}
...
local vars "obid mietekalt nebenkosten baujahr wohnflaeche etage anzahletagen zimmeranzahl immobilientyp year balkon garten keller ausstattung heizungsart kategorie_Haus kategorie_Wohnung"
...
sort ergg_1km
merge m:1 ergg_1km using "../grid.coordinaten.dta"
tab _merge
keep if _merge==3
drop _merge
sort erg_amd
...
gen seCBD =.
forval num = 1/141{
reghdfe lrentsqm ldist Mfloorspace Dfloorspace Mrooms Drooms Mtype Dtype Mbalcony Dbalcony Mgarden Dgarden Mbasement Dbasement Mheating_type i.heating_type if region == `num', abs(FE=year)
...
// PLOT CBD GRADIENTS
preserve
reg lrentsqm ldist Mfloorspace Dfloorspace Mrooms Drooms Mtype Dtype Mbalcony Dbalcony Mgarden Dgarden Mbasement Dbasement Mheating_type i.heating_type if region == `num', abs(year)
predict resid, res
gen lDB = round(ldist, 0.1)
keep if region ==`num'
collapse (mean) resid (count) W=lrent, by(lDB)
twoway (scatter resid lDB, mcolor(black)) (lfit resid lDB [w=W] , lcolor(red) ), xline(0, lcolor(red) lpattern(dash))  title("RAM = `num'")
graph export "gCBD_`num'.png", width(2400) height(1200) replace
restore
// ADDED ELEMENTS END
}
...
gen lrentindex = .
reghdfe lrentqm i.region#c.ldist Mwohnflaeche Dwohnflaeche Mzimmeranzahl Dzimmeranzahl Mimmobilientyp Dimmobilientyp Mbalkon Dbalkon Mgarten Dgarten Mkeller Dkeller Mheizungsart Dheizungsart, abs(FE = ryid)
*reghdfe lrentqm Mwohnflaeche Dwohnflaeche Mzimmeranzahl Dzimmeranzahl Mimmobilientyp Dimmobilientyp Mbalkon Dbalkon Mgarten Dgarten Mkeller Dkeller Mheizungsart Dheizungsart, abs(FE=year i.region#c.ldist)
